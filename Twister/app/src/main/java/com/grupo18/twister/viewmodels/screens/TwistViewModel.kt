...
import com.grupo18.twister.core.network.services.ImageService
import com.grupo18.twister.core.network.services.TwistService
import com.grupo18.twister.core.services.ImageUploadUtils
import com.grupo18.twister.models.game.TwistModel
import com.grupo18.twister.main.MyApp
import com.grupo18.twister.models.network.responses.UploadResponse
...
class TwistViewModel(private val myApp: MyApp) : ViewModel() {
    ...
    private val imageService: ImageService = RetrofitClient.retrofit.create(ImageService::class.java)
    ...
    fun updateTwist(token: String, twist: TwistModel, scope: CoroutineScope, context: Context) {
        scope.launch(Dispatchers.IO) {
            try {
                val response = twistService.editTwist(token, twist).execute()
    ...
    fun deleteTwist(token: String, id: String, scope: CoroutineScope, context: Context) {
        scope.launch(Dispatchers.IO) {
            try {
                ...
                val response = twistService.deleteTwist(token, id).execute()
                if (response.isSuccessful) {
    ...
    fun loadTwists(
        ...
        scope.launch(Dispatchers.IO) {
            try {
                val response = twistService.getUserTwists(token).execute()
                if (response.isSuccessful) {
    ...
    fun uploadImage(context : Context, token: String, imageUri: String, contentResolver: ContentResolver, onResult: (Response<UploadResponse>) -> Unit) {
        val filePart = ImageUploadUtils.prepareImageFile(imageUri)
        filePart?.let {
            val call = imageService.uploadImage(token, it)
    ...
    fun deleteImageFromTwist(token: String, Twist: TwistModel, scope: CoroutineScope, context: Context) {
        scope.launch(Dispatchers.IO) {
            try {
                ...
                val response = imageService.deleteImage(token = token, twistData = Twist).execute()
                if (response.isSuccessful) {
    ...
    fun downloadImagesForTwist(
        ...
        CoroutineScope(Dispatchers.IO).launch {
            ...
            if (false) {
                ...
                val response = imageService.checkImageUpdate(token, imageUri, lastModified).execute()
            ...
            try {
                ...
                val downloadResponse = imageService.downloadImage(token, imageUri).execute()
    ...
    fun loadPublicTwists(
        ...
        scope.launch(Dispatchers.IO) {
            try {
                val response = twistService.getPublicTwists(token).execute()
                if (response.isSuccessful) {
    ...
    fun loadPublicTwistById(
        ...
        viewModelScope.launch(Dispatchers.IO) {
            try {
                val response =twistService.getPublicTwistById(twistId).execute()
                if (response.isSuccessful) {
